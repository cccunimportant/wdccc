...
## 問題：請設計一顆簡易的 CPU，並寫出下列描述內容

(本題的 CPU 設計愈獨特，與老師和其他同學設計愈不同者，分數愈高)

### (a) 請問該 CPU 的暫存器是幾位元的？有哪些暫存器？ (5%)

16 位元，有指令暫存器 IR，程式計數器 PC ，狀態佔存器 SW 與累積器 A 等四個暫存器。

### (b) 請描述該 CPU 的指令集，有那些指令，每個指令的功能為何？ (15%)

參考解答的指令表。

### (c) 請用這些指令的組合語言寫出一個可以計算 sum=1+...+n 的結果的程式，其中的 sum 與 n 都是放在記憶體的變數。 (10%)
...
### (f) 請用 Verilog 設計出該 CPU。

參考解答中的 Verilog 程式

### (g) 請用上述機器碼輸入到 Verilog 程式中進行測試 (可以用 Icarus Verilog)。

參考解答中的執行結果。


## 參考解答：16 位元處理器 CPU16 的設計

### 暫存器：IR, SW, PC, A

指令暫存器 IR，程式計數器 PC ，狀態佔存器 SW 與累積器 A 等四個暫存器。
...
```
D:\Dropbox\Public\web\oc\code>iverilog -o cpu16m cpu16m.v
